...
Если опираться на фразу источника про "на данный момент это 4 метода", то к базовому ядру с высокой вероятностью относятся:

- принимать `POST` запросы с JSON body;
- проверять подпись по полям первого уровня;
- реализовать базовые игровые методы:
  - `/check.session`
  - `/check.balance`
  - `/withdraw.bet`
  - `/deposit.win`
