...
- базовые игровые методы `check.session`, `check.balance`, `withdraw.bet`, `deposit.win` выглядят как ядро server-side взаимодействия;
- recovery-методы `trx.cancel` и `trx.complete` явно описаны и важны для корректной обработки сбоев;
