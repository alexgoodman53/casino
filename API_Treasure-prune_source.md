...
1. `API Платформы` (партнер совершает запросы к серверу платформы) - [API Платформы](https://treasure-prune-f68.notion.site/API-65578958615642d2b5eaa11fc371c0e6)
2. `API Интеграции` (сервер платформы совершает запросы к серверу партнера) - Партнеру необходимо на своей стороне реализовать АПИ, к которому Платформа будет выполнять запросы с целью списания и пополнения счетов Игроков исходя из соответствующих событий (Пример: совершение Игроком ставки или получение выигрыша после окончания раунда в игре) - [API Интеграции](https://treasure-prune-f68.notion.site/API-94ca981a658c4772920afac44fc414a3)
3. `Запуск игр` - Партнеру со своей стороны необходимо реализовать отображение и запуск игр. Доступные варианты для запуска игр описаны: [Запуск игр](https://treasure-prune-f68.notion.site/15eacd5a3c974a788c27bb95ba72942b).
...
- Полный список валют: `http://test.partners.casinomobule.com/currencies.list?partner.alias=test`
