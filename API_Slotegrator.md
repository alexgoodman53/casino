...
**Обязательное operational-требование из PDF**

В самом начале PDF отдельно указано:

- после завершения интеграции нужно передать Slotegrator IP-адреса production-серверов казино;
- эти IP должны быть добавлены в whitelist на стороне Slotegrator;
- если не передать все IP или не уведомить об их изменении после обновления инфраструктуры, API может стать недоступным.
...
- `201` — ресурс создан в ответ на `POST`, URL нового ресурса должен быть в заголовке `Location`;
- `204` — запрос успешно обработан, но тело ответа отсутствует;
- `304` — ресурс не изменился, можно использовать кэшированную версию;
- `400` — некорректный запрос;
- `401` — ошибка аутентификации;
- `403` — доступ к endpoint запрещен;
- `404` — ресурс не найден;
- `405` — метод не поддерживается, нужно смотреть заголовок `Allow`;
- `415` — неподдерживаемый media type или версия контента;
- `422` — ошибка валидации данных;
- `429` — запрос отклонен из-за rate limit;
- `430` — внутренняя серверная ошибка.

**Generic error response**

Базовый формат ошибки в API агрегатора:

- `name` — имя исключения;
- `message` — текст ошибки;
- `code` — код исключения, по умолчанию `0`;
...
- rate limit для `/games`:
  - `staging` и `demo`: до `100` запросов в секунду;
  - `production`: `1` запрос в секунду;
...
- `limits` обычно приходит как массив, но PDF отдельно предупреждает, что некоторые игры могут вернуть **один limit object** вместо массива;
- значит, парсер lobby-ответа должен быть готов к полиморфному полю `limits`, а не только к `limits[]`;
...
- `balance`
- `transaction_id`
- `rollback_transactions` — обязательный массив integrator transaction IDs по всем откатанным операциям

**Самое важное правило**

PDF прямо говорит:

- интегратор в ответе обязан вернуть **все** transaction IDs, связанные с реально обработанным rollback;
- если какой-то transaction ID из rollback-сценария будет пропущен, агрегатор считает rollback response неуспешным;
...
В PDF есть внутренняя нестыковка:

- в таблице response fields поле названо как `provider` с типом `array`;
- в JSON-примере ответа используется ключ `providers`.

Для практической реализации безопаснее ориентироваться на фактический JSON-ответ и отдельно подтвердить это у вендора.
...
Как и в `/limits`, PDF здесь противоречив:

- таблица полей использует имя `provider` с типом `array`;
- JSON-пример использует ключ `providers`.

Для DTO и валидации ответа это лучше явно проверить на sandbox/UAT.
...
Также PDF отдельно предупреждает:

- jackpot values кэшируются на стороне агрегатора `60 секунд`;
- значит, эти данные нельзя считать strictly real-time.
...
- `player_id`
- `title` — строка длиной от `1` до `40` символов
- `currency`
- `initial_balance`
- `max_winnings`
- `valid_until`
- `voucher_id`
- `table_ids` — поле типа `string[]`
- `short_terms`
- `terms_and_conds` — URL со списком условий

**Что важно**

- логическое имя поля в таблице PDF — `table_ids`;
- в примере `application/x-www-form-urlencoded` массив передается как `table_ids[]=...`;
- это важно не смешивать: для бизнес-контракта поле называется `table_ids`, а форма `table_ids[]=` — это способ сериализации массива в request body;
- значения `table_ids` берутся из `tableId`, который приходит в `/games/lobby`.
- Для `valid_until` PDF ссылается на свойство `freevoucher_valid_until_full_day`; это значит, что при реализации нужно отдельно проверить правило формирования даты окончания кампании.
...
> Ниже собраны примеры, которые есть в самом PDF.
>
> Принцип оформления:
>
> - примеры ниже приведены в нормализованном и единообразном виде;
> - сохранены поля, значения и смысл примеров из PDF;
> - служебные строки вроде `HTTP/1.1 200 OK` и визуальные артефакты верстки опущены там, где они не меняют контракт ответа.
...
## 17. Практический вердикт для команды

Ниже короткий operational summary по текущему состоянию документации: что уже можно брать в разработку, что нужно отдельно уточнять у вендора и что обязательно прогонять на sandbox/UAT до production.

| Уже достаточно для разработки | Нужно уточнить у вендора | Критично проверить на sandbox/UAT |
| --- | --- | --- |
| Есть базовый transport contract: `HTTP/1.1`, `application/x-www-form-urlencoded`, JSON responses, auth headers, `X-Sign`, окно `30 сек`. | Как именно вендор ожидает обрабатывать replay по `X-Nonce`: нужна ли проверка уникальности nonce на стороне казино или достаточно timestamp + sign. | Невалидная подпись, просроченный `X-Timestamp`, переставленные параметры, пустые optional-поля. |
| `/games`, `/game-tags`, `/games/lobby`, `/games/init` описаны достаточно, чтобы строить каталог, лобби и launch flow. | Полный список параметров `/games/init-demo`, точные pagination/query-параметры кроме явно видимых, есть ли vendor-рекомендация по TTL кэша. | Кэш каталога, rate limits `/games`, lobby game с массивом `limits` и с single-object `limits`, mobile/desktop launch. |
| Денежный callback contour описан достаточно для backend: `balance`, `bet`, `win`, `refund`, `rollback`, idempotency, `round_id`, delayed refund, rollback only by list. | Точный retry schedule: интервалы, сколько реально повторов по каждому action, всегда ли приходит `casino_request_retry_count`, для каких мерчантов включены optional поля. | Дубликаты `bet/win/refund/rollback`, `refund` раньше исходного `bet`, rollback на часть раунда, ELK freespin win без `round_id`, смена `game_uuid` при том же `session_id`. |
| Формат callback error response понятен: всегда `HTTP 200`, плюс `error_code`/`error_description`. | Полная бизнес-матрица `error_code` по всем методам, кроме явно указанных `INSUFFICIENT_FUNDS` и `INTERNAL_ERROR`. | Что делает агрегатор на каждый тип ошибки: повтор, refund, stop flow; как обрабатываются пустые `error_description`. |
| Freespins API уже хватает, чтобы реализовать CRUD кампании и привязку ставок через `bet_id + denomination` или `total_bet_id`. | Полный lifecycle статусов кампании, retry/compensation semantics для promo-потока, точное правило `freespin_valid_until_full_day`. | Создание кампании обоими способами, граничные даты `valid_until`, cancel после частичного использования, сверка `quantity_left` и `total_win`. |
| Freevouchers API уже достаточно описан для основной реализации: `title 1..40`, `table_ids[]: string[]`, `terms_and_conds` как URL, `voucher_id`, `playable`, `winnings`. | Допустимые значения `state`, правило `freevoucher_valid_until_full_day`, точная валидация `short_terms` и URL-поля, можно ли передавать несколько `table_ids` для одной кампании у всех провайдеров. | Валидация длины `title`, битый URL в `terms_and_conds`, пустой/невалидный `table_ids[]`, поведение `playable` и `winnings` после частичного использования и cancel. |
| `self-validate` и legacy endpoints описаны достаточно, чтобы понимать их место в интеграции. | Точный request body `/self-validate`, обязателен ли `/balance/notify` для данного мерчанта, нужны ли вообще `LEGACY`-методы в новой интеграции. | Прогон `self-validate` на свежей сессии, сверка логов, отдельное подтверждение от менеджера Slotegrator по legacy-методам до go-live. |

Если сжать вывод до одного абзаца: документации уже хватает, чтобы начать backend и wallet-интеграцию, но перед production обязательно нужно письменно закрыть у вендора четыре зоны риска:

1. полный контракт `/games/init-demo`;
2. полная error matrix по callback-методам;
3. retry policy для денежных callbacks;
4. точные правила дат и статусов для freespins/freevouchers кампаний.
