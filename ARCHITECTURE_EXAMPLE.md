...
- `Provider Policy`
  ...
  Почему этот слой реально нужен на практике:
  если его не сделать, то эти правила придется складывать либо в adapter, либо прямо в engine.
  Тогда:
  - adapter станет слишком "умным" и начнет принимать бизнес-решения;
  - engine начнет обрастать `if provider == ...` и станет ломаться от частных исключений.
  Реальный пример на Slotegrator из этого проекта:
  - у него `refund` может прийти раньше, чем система найдет исходную `bet`;
  - у него `bet` и `win` могут прийти с разными `session_id`, но с одним `round_id`;
  - у него rollback надо делать только по списку `rollback_transactions`, а не "по всему round";
  - у него очень короткий callback budget около `3 секунд`.
  Это уже не просто parsing запроса, потому что здесь есть правила поведения.
  Но это и не общая логика всех seamless API, потому что такие детали завязаны именно на Slotegrator.
  Поэтому policy как раз и говорит engine:
  - "для этого провайдера round ищем в первую очередь по `round_id`";
  - "`refund` здесь может быть normal-case раньше source bet";
  - "rollback можно делать только по тому списку transaction ids, который пришел извне";
  - "если обработка слишком долгая, провайдер может быстро уйти в retry/recovery сценарий".
  Маленький пример:
  пришел `refund` от Slotegrator, а `bet` еще не найдена.
  Adapter только распарсил запрос.
  Policy объяснила, что это допустимый сценарий именно для этого API.
  После этого engine уже понимает, что событие не надо считать ошибкой:
  его нужно положить в `pending_operations` и ответить успехом.
  Для Treasure-prune policy нужна по другой причине:
  она знает, что после `withdraw.bet` или `deposit.win` могут прийти отдельные `trx.cancel` и `trx.complete`.
  Значит для этого провайдера важно не только само денежное событие, но и его дальнейшая судьба после сбоя.
