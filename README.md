...
Вывод из сравнения простой:

- `Treasure-prune` сложнее по жизненному циклу транзакции;
- `Slotegrator` сложнее по edge-cases обычного seamless-потока;
- значит у них разный центр тяжести в архитектуре, даже если обе интеграции в итоге упираются в один и тот же ledger казино.
...
Нужно строить не "один оркестратор" и не "много толстых адаптеров", а **модульную платформу интеграций**.
...
### 8.1. Короткий итог по двум интеграциям

| API | Подходящий движок | Главный риск | Что важно не смешивать |
| --- | --- | --- | --- |
| `Treasure-prune` | `StatefulTransactionEngine` | `cancel/complete` после основной денежной операции | real-money flow и feature-dependent расширения |
| `Slotegrator` | `SeamlessWalletEngine` | `refund before bet`, пакетный `rollback`, неочевидная связь `session_id` и `round_id` | базовый money-flow и promo/legacy/self-validation контуры |

### 8.2. Что точно не нужно делать

- отдельный mini-core внутри каждого адаптера;
- отдельные таблицы денег под каждого провайдера;
- отдельную логику баланса в каждом адаптере;
- отдельную трактовку раунда в обход общего ядра.

### 8.3. Когда допустим "средний адаптер"

Средний адаптер допустим, если он:

- знает поля и edge-cases провайдера;
- умеет выбрать нужную policy;
- не двигает деньги сам;
- не хранит свой money-ledger;
- не вводит свою идемпотентность отдельно от общей платформы.

Практический компромисс такой:

- не делать адаптер тупым любой ценой;
- не делать из адаптера отдельное денежное ядро;
- держать `Adapter + Provider Policy` поверх общих движков и общего ledger.
...
### 10.2. Короткие сценарии

- `Treasure-prune withdraw.bet`
  Адаптер разбирает запрос, policy определяет real-money flow, `StatefulTransactionEngine` проверяет дубли и вызывает core, после чего сохраняет внешний статус.

- `Treasure-prune trx.complete`
  Адаптер разбирает контейнерный вызов, policy находит исходный flow, `StatefulTransactionEngine` завершает внешний lifecycle без повторного движения денег, если money-effect уже был применен.

- `Slotegrator refund`
  Адаптер разбирает `action=refund`, policy подтверждает допустимость orphan-сценария, `SeamlessWalletEngine` либо делает компенсацию, либо кладет событие в `pending_operations`.

Подробный пример классов, таблиц и flow вынесен в [ARCHITECTURE_EXAMPLE.md](./ARCHITECTURE_EXAMPLE.md).
